...
export const VideoPlayerView: React.FC<VideoPlayerViewProps> = ({
  ...
  useEffect(() => {
    if (currentLesson && currentLesson.id) {
      ...
      if (currentLesson.form === 5 && !hasAccessToForm5) {
  ...
  useEffect(() => {
    if (!currentLesson || !currentLesson.id || currentLesson.isPendingUpload || (currentLesson.form === 5 && !hasAccessToForm5)) return;
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs font-black text-emerald-300 tracking-wide flex items-center gap-1.5">
                  ...
                  <span className="text-emerald-100 font-semibold">
                    {devSimulateLocked 
                      ? (lang === "bm" ? "Sedang Menguji Mod Terkunci (Simulasi Pelajar)" : "Testing Locked Mode (Student Simulation)")
                      : (lang === "bm" ? "Akses Video T5 Dibuka Penuh" : "Full F5 Video Access Unlocked")}
                  </span>
                </span>
              </div>

              <div className="flex items-center gap-2 flex-wrap">
                {/* Butang Terus Buka Modal Bayaran RM 1.99 */}
                <button
                  type="button"
                  onClick={() => setShowCheckoutModal(true)}
                  className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-amber-500 via-amber-600 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-slate-950 text-xs font-black flex items-center gap-1.5 shadow-md shadow-amber-500/25 active:scale-95 cursor-pointer ring-1 ring-amber-300/40"
                >
                  <Crown className="w-3.5 h-3.5 fill-slate-950" />
                  <span>{lang === "bm" ? "Uji Bayar FPX (RM 1.99)" : "Test FPX Pay (RM 1.99)"}</span>
                  <Zap className="w-3 h-3 fill-slate-950" />
                </button>

                {/* Butang Tukar Paparan Terkunci (Simulasi) */}
                <button
                  type="button"
                  onClick={() => setDevSimulateLocked(prev => !prev)}
                ...
                >
                  ...
                  <span>{devSimulateLocked ? (lang === "bm" ? "Buka Semula" : "Unlock Back") : (lang === "bm" ? "Uji Paparan Terkunci" : "Simulate Lock")}</span>
                </button>

                <span className="text-[10px] font-mono text-emerald-400/80 bg-emerald-950/80 px-2 py-1 rounded border border-emerald-500/30">
                  {userEmail}
                </span>
              </div>
            </div>
